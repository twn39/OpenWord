...
Paragraph::Paragraph(void *node) : node_(node) { Expects(node_ != nullptr); }
...
Paragraph &Paragraph::setIndentation(int leftTwips, int rightTwips, int firstLineTwips, int hangingTwips) {
    ...
    if (firstLineTwips > 0)
        ind.append_attribute("w:firstLine").set_value(std::to_string(firstLineTwips).c_str());
    if (hangingTwips > 0)
        ind.append_attribute("w:hanging").set_value(std::to_string(hangingTwips).c_str());
    return *this;
}
...
Section Paragraph::appendSectionBreak() {
    ...
    if (!pSectPr)
        pSectPr = pPr.append_child("w:sectPr");
    else
        pSectPr.remove_children(); // Ensure clean slate
...
void Paragraph::addImage(gsl::czstring image_path, double scale, ImagePosition position, long long xOffset,
                         long long yOffset) {
    auto n = cast_node(node_);
    ...
    if (position != ImagePosition::Inline) {
        ...
        graphicFrameLocks.append_attribute("xmlns:a").set_value(
...
Run Paragraph::addHyperlink(gsl::czstring text, gsl::czstring url) {
    ...
    if (!h_store)
        h_store = root.append_child("openword_hyperlinks");
...
int Paragraph::replaceText(const std::string &search, const std::string &replace) {
    ...
    while (true) {
        ...
        for (auto &tr : runs) {
            ...
            if (r_end <= match_pos || r_start >= match_end)
                continue;
            ...
            if (!new_val.empty() &&
                (new_val.front() == ' ' || new_val.back() == ' ' || new_val.find('\t') != std::string::npos ||
                 new_val.find('\n') != std::string::npos)) {
...
void Paragraph::addEquation(const std::string &omml) {
    ...
    if (!math_doc.load_string(omml.c_str()))
        return;

    auto math_node = math_doc.child("m:oMath");
    if (!math_node)
        math_node = math_doc.child("m:oMathPara");
...
static void applyParaBorder(pugi::xml_node borders, const char *borderName, const BorderSettings &bs) {
    ...
    b.append_attribute("w:val") = styleStr;
    b.remove_attribute("w:sz");
    b.append_attribute("w:sz") = std::to_string(bs.size).c_str();
    b.remove_attribute("w:space");
    b.append_attribute("w:space") = "1"; // Default 1 pt spacing for para borders
    b.remove_attribute("w:color");
    b.append_attribute("w:color") = bs.color.c_str();
}

Paragraph &Paragraph::setBorders(const BorderSettings &top, const BorderSettings &bottom, const BorderSettings &left,
...
Paragraph &Paragraph::setBorders(const BorderSettings &all) { return setBorders(all, all, all, all); }
...
TextBox Paragraph::addTextBox(long long widthEMU, long long heightEMU, long long xOffsetEMU, long long yOffsetEMU) {
    ...
    std::string style_str = "position:absolute;margin-left:" + std::to_string(xPt) +
                            "pt;margin-top:" + std::to_string(yPt) + "pt;width:" + std::to_string(widthPt) +
                            "pt;height:" + std::to_string(heightPt) + "pt;z-index:251658240;";
...
TextBox::TextBox(void *node) : node_(node) { Expects(node_ != nullptr); }
...
int Paragraph::listLevel() const {
    ...
    if (numPr) {
        auto ilvl = numPr.child("w:ilvl");
        if (ilvl)
            return ilvl.attribute("w:val").as_int(0);
    }
    return 0;
}
