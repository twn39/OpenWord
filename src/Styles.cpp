...
Font& Font::setBold(bool val) {
    ...
    auto b = n.child("w:b");
    if (val) {
        if (!b) n.append_child("w:b");
        auto bCs = n.child("w:bCs");
        if (!bCs) n.append_child("w:bCs");
    } else {
        n.remove_child("w:b");
...
Font& Font::setItalic(bool val) {
    ...
    auto i = n.child("w:i");
    if (val) {
        if (!i) n.append_child("w:i");
        auto iCs = n.child("w:iCs");
        if (!iCs) n.append_child("w:iCs");
    } else {
        n.remove_child("w:i");
...
Font& Font::setName(const std::string& ascii) {
    ...
    if (!rFonts) rFonts = n.append_child("w:rFonts");
...
ParagraphFormat& ParagraphFormat::setOutlineLevel(int level) {
    ...
    if (!outline) outline = n.append_child("w:outlineLvl");
...
ParagraphFormat& ParagraphFormat::setSpacing(int beforeTwips, int afterTwips) {
    ...
    if (!spacing) spacing = n.append_child("w:spacing");
...
Font Style::getFont() {
    ...
    if (!rPr) rPr = n.append_child("w:rPr");
...
ParagraphFormat Style::getParagraphFormat() {
    ...
    if (!pPr) pPr = n.append_child("w:pPr");
