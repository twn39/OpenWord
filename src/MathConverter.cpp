...
std::string sanitize_mathml(const std::string& input) {
    ...
    if (math) math.append_attribute("xmlns:mml") = "http://www.w3.org/1998/Math/MathML";
    ...
    auto remove_all = [&](const std::string& target) {
        size_t pos = 0;
        while ((pos = str.find(target, pos)) != std::string::npos) str.erase(pos, target.length());
    };

    const char* invisible_seqs[] = {
        "\xE2\x81\xA1", "\xE2\x81\xA2", "\xE2\x81\xA3", "\xE2\x81\xA4",
        "&#x2061;", "&#x2062;", "&#x2063;", "&#x2064;",
        "&ApplyFunction;", "&InvisibleTimes;"
    };
    for (const char* seq : invisible_seqs) remove_all(seq);
...
void process_omml_node(pugi::xml_node node) {
    ...
    static const std::set<std::string> containers = {"m:e", "m:sub", "m:sup", "m:num", "m:den", "m:deg", "m:lim", "m:f"};

    // 1. Operator Base Healing: Pull ALL mathematical siblings into empty m:e
    if (name == "m:nary" || name == "m:limLow" || name == "m:limUpp" || name == "m:sSubSup" || name == "m:sSub" || name == "m:sSup") {
        ...
        if (e && !e.first_child()) {
            ...
            while (sib) {
                ...
                if (sib.type() == pugi::node_element || 
                   (sib.type() == pugi::node_pcdata && std::string(sib.value()).find_first_not_of(" \t\n\r") != std::string::npos)) {
                    e.append_copy(sib);
                }
    ...
    if (name == "m:r") {
        ...
        if (!t || std::string(t.child_value()).empty()) {
            ...
            if (next) process_omml_node(next);
            return;
        }
    }
...
std::string sanitize_omml(const std::string& raw_omml) {
    pugi::xml_document doc;
    if (!doc.load_string(raw_omml.c_str())) return raw_omml;
