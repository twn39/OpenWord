...
Section &Section::setPageSize(uint32_t w_twips, uint32_t h_twips, Orientation orient) {
    ...
    if (!pgSz)
        pgSz = n.append_child("w:pgSz");

    auto set_attr = [&](const char *name, int val) {
        if (!pgSz.attribute(name))
            pgSz.append_attribute(name) = std::to_string(val).c_str();
        else
            pgSz.attribute(name).set_value(std::to_string(val).c_str());
    };
    ...
    if (orient == Orientation::Landscape) {
        if (!pgSz.attribute("w:orient"))
            pgSz.append_attribute("w:orient") = "landscape";
        else
            pgSz.attribute("w:orient").set_value("landscape");
    } else {
...
Section &Section::setMargins(const Margins &margins) {
    ...
    if (!pgMar)
        pgMar = n.append_child("w:pgMar");

    auto set_attr = [&](const char *name, int val) {
        if (!pgMar.attribute(name))
            pgMar.append_attribute(name) = std::to_string(val).c_str();
        else
            pgMar.attribute(name).set_value(std::to_string(val).c_str());
    };
...
Section &Section::setColumns(int count, int spaceTwips) {
    ...
    auto &colsNode = cols;
    if (!colsNode.attribute("w:num"))
        colsNode.append_attribute("w:num") = std::to_string(count).c_str();
    else
        colsNode.attribute("w:num").set_value(std::to_string(count).c_str());

    if (!colsNode.attribute("w:space"))
        colsNode.append_attribute("w:space") = std::to_string(spaceTwips).c_str();
    else
        colsNode.attribute("w:space").set_value(std::to_string(spaceTwips).c_str());
...
openword::Section &openword::Section::removeFooter(HeaderFooterType type) {
    auto n = cast_node(node_);
    std::string type_str = (type == HeaderFooterType::First)  ? "first"
                           : (type == HeaderFooterType::Even) ? "even"
                                                              : "default";
